...
def generate_test_suite(config_path: Path, cif_path: Path, output_path: Path | None = None) -> None:
    ...
    if output_path.exists():
        raise FileExistsError(
            f"Output file '{output_path}' already exists. Please delete it or specify a different output path."
        )
    ...
    for command in commands:
        ...
        for param in parameters:
            ...
            if dtype == "QCrBox.cif_data_file":
                ...
                required_params.append({"name": param_name, "type": param_type, "value": default_value})
            else:
                ...
                todos.append({"command": command_name, "parameter": param_name, "type": dtype or "unknown"})
        ...
        if cif_param_name:
            test_case = {
                ...
                "timeout_seconds": 60,
                "input_parameters": [{"name": cif_param_name, "type": "external_file", "value": relative_cif_path}]
                + required_params,
                "expected_results": [{"result_type": "status", "expected": "successful"}],
            }
