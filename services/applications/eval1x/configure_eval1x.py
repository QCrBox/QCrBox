...
def integrate(
    ...
    create_reflection_cif(
        work_folder
    )

    final_cell_refinement(
        work_folder,
        rmat_file_path
    )
...
def create_shoes(
    ...
    work_folder = pathlib.Path(work_folder)
    if rmat_file_path == 'work_folder':
        rmat_file_path = next(reversed(sorted(
            work_folder.glob('*.rmat'), key=os.path.getmtime
        )))
    if beamstop_file_path == 'work_folder':
        beamstop_file_path = next(reversed(sorted(
            work_folder.glob('beamstop.vic'), key=os.path.getmtime
        )))
    if detalign_file_path == 'work_folder':
        detalign_file_path = next(reversed(sorted(
            work_folder.glob('detalign.vic'), key=os.path.getmtime
        )))
...
def final_cell_refinement(
    ...
    peakref = EvalPeakrefRobot(
        work_folder=work_folder,
        rmat_file=RmatFile.from_file(rmat_file_path)
    )

    peakref.refine_parameters(
        'ic/final',
        new_rmat_filename='ic_refined.rmat',
        end_with_cell=True
    )
