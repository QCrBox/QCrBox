        ...
        with settings.db.get_session() as session:
            ...
            try:
                application = session.exec(
                    select(sql_models_NEW_v2.ApplicationSpecDB).where(
                        sql_models_NEW_v2.ApplicationSpecDB.slug == self.application_slug,
                        sql_models_NEW_v2.ApplicationSpecDB.version == self.application_version,
                    )
                ).one()
