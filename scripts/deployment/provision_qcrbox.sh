...
ADMIN_HASH=$(docker run --rm authelia/authelia:4.38 \
    ...
    | sed -n 's/^Digest: //p')
[ -n "$ADMIN_HASH" ] || { echo "ERROR: failed to generate password hash" >&2; exit 1; }
...
echo "==> Writing users database (user: admin)"
cat > "$QCRBOX_DIR/services/core/qcrbox_auth/users_database.yml" <<EOF
---
###############################################################
#                      Users Database                         #
###############################################################
# Generated by provision_qcrbox.sh. Add users with:
# docker run --rm authelia/authelia:4.38 authelia crypto hash generate argon2 --password '...'

users:
  admin:
    displayname: "QCrBox Admin"
    password: "$ADMIN_HASH"
    email: admin@$DOMAIN
    groups:
      - admins
...
# ------------------------------------------------------------ start backend
# Old Authelia state is unreadable after rerolling the storage encryption key.
docker volume rm -f qcrbox_qcrbox-authelia-data >/dev/null 2>&1 || true
...
Web login (Authelia, also Django admin):
  username: admin
