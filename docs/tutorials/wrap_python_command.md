...
The initial setup, generated by `qcb init`, has already populated the top section of the YAML file. Your task now is to customize this section with our specific command details. Start by deleting the first command and renaming the second one by changing `name` from the `"sample_cmd_v2"` placeholder to `"get_number_fitting_cod_entries"`. Leave `implemented_as` to be `"python_callable"`. The module we will import is defined in the next line, so leave it as `import_path: "qcb_configure_cod_check_tutorial"`. Also delete the `cif_entry_sets` section at the end for the time being.
...
Next, we must identify which CIF entries in which ddl convention have to be in the input cif file for our command to function. Inspect the `cif_to_search_pars` function to determine these entries. If you're adding only one command, list these required entries directly in the input_cif_path parameter definition. Ensure that `required_entries:` aligns with the `name:` and `type:` sections of that parameter for proper structure.
...
Let's proceed to replace the boilerplate function with the necessary Python functions within `configure_cod_check_tutorial.py`:
...
## Keep the boilerplate starting the QCrBox Application

Make sure that the following code at the end of the configure file is still present.

```python
if __name__ == "__main__":
    application_spec = sql_models_NEW_v2.ApplicationSpec.from_yaml_file(
        "config_cod_check_tutorial.yaml"
    )
...
6. **Delete unnecessary lines**: The following three lines/sections are not necessary for a Python application and should be deleted:
    ```Dockerfile
    USER root
...
```bash
qcb up cod_check_tutorial --no-build
```
