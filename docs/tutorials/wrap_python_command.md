...
This guide walks you through the process of encapsulating a Python module within a QCrBox container. Specifically, we'll focus on a module that queries the [Crystallographic Open Database (COD)](https://www.crystallography.net/cod/) for structures with similar elements and unit cell parameters. Our goal is to make this module's functionality accessible within a QCrBox container. The resulting module is already present in QCrBox as `cod_check`. However, we will go through the necessary steps to recreate the functionality.
...
To begin, open the `configure_cod_check_tutorial.py` file. Start by importing necessary functions from the Python base libraries as well as two different modules:
...
Finally, `invalidated_entries` are entries that are no longer valid with a transformation. In the given example we have changed atom positions and displacement parameters. Accordingly, anything depending on these parameters should not be kept from the original input cif file. We should delete all derived `_geom` parameters and all quality indicators (as out diffraction data might be different from the source). Invalidated entries should be given to match regular expressions as used by the [python re module](https://docs.python.org/3/library/re.html).
