...
1. Modify or remove `description` as you wish.
...
If you have not done so you can shut down QCrBox by typing `qcb down`. You can now restart and rebuild the container by typing the following. Rebuilding without dependencies (using the `--no-deps` command flag stated earlier) might be faster if you have just rebuilt everything.
