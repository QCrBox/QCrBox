...
def get_qcrbox_registry_api_connection_url(
    *,
    ...
    host = host or os.environ.get("QCRBOX__QCRBOX_REGISTRY_HOST", "127.0.0.1")
    port = port or int(os.environ.get("QCRBOX__QCRBOX_REGISTRY_PORT", 11000))
...
def get_container_qcrbox_id():
    ...
    if container_qcrbox_id_file.exists():
        logger.debug(f"Reading qcrbox_container_id from file: {container_qcrbox_id_file.as_posix()}")
        with container_qcrbox_id_file.open() as f:
            container_qcrbox_id = f.read()
